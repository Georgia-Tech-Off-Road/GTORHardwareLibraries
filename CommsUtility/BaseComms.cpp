...
const byte& end_code_t::operator[] (uint8_t i) const {
    return code[i];
}
...
void BaseComms::packetize() {
    ...
    if(_is_sending_data){
        ...
        for(auto it = _transmit_blocks.begin(); it != _transmit_blocks.end(); it++){
            uint8_t *pack = new byte[(*it)->get_pack_bytes()];
            (*it)->pack(pack);
        ...
        for(auto it = _transmit_blocks.begin(); it != _transmit_blocks.end(); it++){
            ...
            uint8_t pack_bytes = (*it)->get_pack_bytes();
...
uint8_t BaseComms::get_expected_receive_bytes() {
    uint8_t bytes = 0;
    for(auto it = _received_blocks.begin(); it != _received_blocks.end(); it++) bytes += (*it)->get_pack_bytes();
    return (9 + bytes);
}

uint8_t BaseComms::get_expected_transmit_bytes() {
    uint8_t bytes = 0;
    for(auto it = _transmit_blocks.begin(); it != _transmit_blocks.end(); it++) bytes += (*it)->get_pack_bytes();
    for(auto tp = _throughput_comms.begin(); tp != _throughput_comms.end(); tp++){
        for(auto it = (*tp)->_received_blocks.begin(); it != (*tp)->_received_blocks.end(); it++){
            bool detached = 0;
            uint16_t id = (*it)->get_id();
 ...
 */
void BaseComms::attach_throughput_uart(BaseComms &throughput_comms){
    // Check to see if it is already attached. If so, do nothing
